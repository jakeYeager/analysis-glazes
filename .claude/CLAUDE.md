...
- **`Ferro Frit 3124`** (from "Looks Expensive") is still unpriced (`not_found`), but `find_material_candidates.py` already found a strong candidate — IMCO SKU 26800 ("Frit 3124") — by trying the manufacturer-prefix-stripped name. Worth pricing and confirming; same pattern as the now-resolved `Ferro Frit 3134` → `Frit 3134` case (Run 8).
- The scraping scripts were verified end-to-end against live Glazy/IMCO pages on 2026-07-04 (see `docs/price_list_run_notes.md` Runs 4-8). Gotchas worth knowing if you touch these scripts: (1) `wait_until="networkidle"` hangs on IMCO's Square Online pages (persistent background connections never go idle) — use `"load"` + a short fixed wait instead; (2) IMCO's search doesn't return zero results for a bogus query, it falls back to unrelated "trending" products, so `find_material_candidates.py` filters matches by word-overlap with the search term rather than trusting raw hit counts; (3) `import_glazy_recipe.py`'s `is_addition` and `firing_type` detection are confirmed live against a real raku recipe with additions (Run 7) — see that script's docstring for the corrected "Total base recipe" subtotal-row logic (the original `"+"`-prefix guess was wrong); (4) manufacturer-prefix name variants (e.g. "Ferro Frit NNNN" = "Frit NNNN") are a confirmed class, not a guess — see "Material name variants" in `.claude/rules/conventions.md` (Run 8).
