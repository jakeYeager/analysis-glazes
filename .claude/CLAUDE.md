...
- **Resolved (2026-07-10):** three mid-fire recipes verified by Joel (glaze tech) as matching studio use, from `_inbox/from_joel.md`. Falls Creek Shino (`glazy.org/recipes/7965`, $5.89/lb) and Buttercup Yellow (`glazy.org/recipes/24859`, $3.63/lb) were new imports via the normal `import_glazy_recipe.py` flow, both with real `is_addition` splits read directly off Glazy's subtotal row. Frogskin (`glazy.org/recipes/292795`, already tracked, $2.40/lb, unchanged) is the odd one out: its Glazy page has no "Total base recipe" subtotal row at all — re-confirmed live via `--force` re-import — so unlike every other recipe here, its base/addition split can't be read off the page directly. Per Joel and studio convention (colorants/opacifiers are additions on a 100-part base), it was deduced by material chemistry instead: Copper Carbonate, Red Iron Oxide, and Titanium Dioxide (carbonate/oxides) marked `is_addition`, the remaining six (frit-free base of Silica/Gerstley Borate/Nepheline Syenite/Potash Feldspar/EP Kaolin/Wollastonite, 98 parts) left as base. This only affects reporting/classification, not `$/lb` math (which sums all ingredients regardless), so Frogskin's price is unchanged.
  - Two new materials confirmed and priced: `Spodumene` (IMCO SKU 720, exact, single unambiguous hit) and `Dolomite` (IMCO SKU 769, exact, single unambiguous hit), both new to Buttercup Yellow.
  - `Alberta Slip` (new, Falls Creek Shino) priced via IMCO's "Alberta Slip - Substitute for Albany Slip" (SKU 25120, `match_confidence='exact'`) — this is IMCO's literal product name, not an inferred substitution: Alberta Slip *is* the modern replacement for the discontinued Albany Slip, so the naming is accurate, not a mismatch. Sold in a 44 lb bag (no true 50lb tier); bucketed to `price_50lb` per the standard non-standard-pack-size convention, disclosed in the material's `notes`.
  - Buttercup Yellow's "Kentucky OM #4 Ball Clay" was merged into the existing `Ball Clay` material rather than tracked as a new one — Old Mine #4 is sourced from the Kentucky-Tennessee ball clay district and IMCO's own product title is just "OM4 Old Mine #4 Ball Clay" (no separate Kentucky-branded SKU), so this is the same grade already priced for Hasselle Copper Matte/Reynolds Wrap, just a more descriptive name from this particular recipe source.
...
- **`Tennessee Ball Clay`** (new material, used only by Ferguson's White Crackle) is a distinct grade from the generic `Ball Clay`/OM4 above — Ferguson's recipe specifies it by name. Priced 2026-07-05 via IMCO's single "Tennessee #1 Ball Clay" product, `match_confidence='exact'` (one unambiguous hit, no grade ambiguity to spot-check).
...
- **Resolved:** "Del Favero Luster" confirmed 2026-07-05 against `glazy.org/recipes/836250` — normal `import_glazy_recipe.py --force` re-import (published, name matched, no auth needed). Ingredients matched exactly (no CMC-Gum-style variance here). `is_addition` split applied: Gerstley Borate/Nepheline Syenite are base, Copper Carbonate is the addition. New total $396.80/102 lb = $3.89/lb.
- **Resolved:** "Fern Green Crackle" confirmed 2026-07-05 against `glazy.org/recipes/836239` — normal `import_glazy_recipe.py --force` re-import (published, name matched, no auth needed). Ingredients matched exactly. `is_addition` split applied: Gerstley Borate/Nepheline Syenite are base, Chrome Oxide is the addition. New total $379.70/100.50 lb = $3.78/lb.
- **Resolved:** "Sky Blue Crackle" confirmed 2026-07-05 against `glazy.org/recipes/836251` — normal `import_glazy_recipe.py --force` re-import (published, name matched, no auth needed). Ingredients matched exactly. `is_addition` split applied: Gerstley Borate/Nepheline Syenite are base, Cobalt Carbonate is the addition. New total $386.80/100.50 lb = $3.85/lb.
- **Resolved:** "Ferguson's White Crackle" confirmed 2026-07-05 against `glazy.org/recipes/836306` (renamed from spreadsheet "Ferguson White Crackle" to match Glazy's title, which added the apostrophe-s and credits "15 Raku Glazes"). This recipe is **unpublished** (Jake hasn't added a photo yet, required by Glazy to publish) — viewed via a logged-in Chrome session and applied by hand, same workaround as Reynolds Wrap. `is_addition` split applied: Gerstley Borate/Nepheline Syenite/Silica/Tennessee Ball Clay are base, Tin Oxide is the addition. **Discovered the recipe specifies "Tennessee ball clay" explicitly, not generic Ball Clay** — this material was pulled out into its own `Tennessee Ball Clay` row (see above) rather than reusing the OM4-priced `Ball Clay` material, since that would have silently priced the wrong grade. New total $606.60/100 lb = $6.07/lb.
- **Resolved:** "Clear Crackle" confirmed 2026-07-05 against `glazy.org/recipes/561444` — normal `import_glazy_recipe.py --force` re-import (published, name matched, no auth needed). Ingredients matched exactly. No "Total base recipe" subtotal row at all (goes straight to "Total") — every ingredient is base, no additions, matching the `is_addition=false` default already in place. New total $372.80/100 lb = $3.73/lb.
- **Resolved:** "Marble White Crackle" confirmed 2026-07-05 against `glazy.org/recipes/836320` — same name, ingredients matched exactly. This recipe is **unpublished** (Jake hasn't added a photo yet, required by Glazy to publish) — viewed via a logged-in Chrome session and applied by hand, same workaround as Reynolds Wrap and Ferguson's White Crackle. `is_addition` split applied: Gerstley Borate/Nepheline Syenite are base, Zinc Oxide/Tin Oxide are additions. New total $519.20/110 lb = $4.72/lb.
- **Resolved:** "Ballingham Black Luster" confirmed 2026-07-05 against `glazy.org/recipes/836323` — same name, unpublished (Jake hasn't added a photo yet) — viewed via a logged-in Chrome session and applied by hand. **The spreadsheet's "Custer Feldspar" is "G-200 Feldspar" on the real Glazy page** — Jake had already updated his own recipe to name the real available substitute directly, so this ingredient is priced as the plain `Potash Feldspar` material (same SKU) rather than the `Custer Feldspar` row used for Forbes Midnight Blue, which is a different situation (that recipe is locked and still literally says "Custer Feldspar"). `is_addition` split applied: Gerstley Borate/Nepheline Syenite/Potash Feldspar are base, Copper Carbonate/Black Iron Oxide/Black Copper Oxide/Red Iron Oxide/Tin Oxide/Bentonite/Zircopax/Cobalt Carbonate are additions. New total $700.07/133.50 lb = $5.24/lb. This closes out the entire bulk-import backlog — all 16 original recipes are now accounted for (14 confirmed, 2 dropped).
...
- **Resolved:** "Blue Moon" was **dropped entirely** 2026-07-05, per Jake — it was already flagged `REFERENCE ONLY` (his personal preference, not actively mixed) and he decided it's expensive and redundant with the studio's other blue/green raku glazes, so not worth keeping even for reference. Deleted from `recipes`/`recipe_ingredients`. This closes out the last recipe that was still unconfirmed on `is_addition` under the reference-only carve-out — every remaining recipe on the pending list above is a normal active-rotation recipe.
